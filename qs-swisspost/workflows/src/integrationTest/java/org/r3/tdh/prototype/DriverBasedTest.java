...
public class DriverBasedTest {
    private final TestIdentity rouA = new TestIdentity(new CordaX500Name("ROUA", "", "GB"));
    ...
    public void nodeTest() {
        driver(new DriverParameters().withIsDebug(true).withStartNodesInProcess(true), dsl -> {
            ...
            List<CordaFuture<NodeHandle>> handleFutures = ImmutableList.of(
                    dsl.startNode(new NodeParameters().withProvidedName(rouA.getName()))
...
//                NodeHandle partyBHandle = handleFutures.get(1).get();
                ...
                assertEquals(partyAHandle.getRpc().wellKnownPartyFromX500Name(rouA.getName()).getName(), rouA.getName());
